...
class HbaseClient(c:Connection) {
    ...
    def readObject[E](tn: TableName, rowkey: Array[Byte])(implicit conv: Map[(String, String), Array[Byte]] => E): Option[E] = {
        ...
        if (c.getAdmin.tableExists(tn)) {
            ...
            // parcours de mapasScala pour créer la map mapRes

            /*mapasScala.foreach(colFam =>
              colFam._2.foreach(colQualifier =>
                mapRes+=((Bytes.toString(colFam._1), Bytes.toString(colQualifier._1)) -> colQualifier._2)
               )
            )*/
