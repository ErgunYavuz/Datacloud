...
compile="false"
type=""
DIRECTORY_SRC="$WORKING_DIRECTORY/src"
DIRECTORY_LIB="$WORKING_DIRECTORY/lib"
...
while true; do 
    case "$1" in 
        ...
        -t|--type)
            ...
            required_jar="$DIRECTORY_LIB/datagenerator.jar?datagenerator.Main"
            target_jar="$DIRECTORY_LIB/datagenerator.jar"
            classpath=""
            if [ "$type" = "hbaseschema" ]
            then
            	[ "`env | grep MY_HBASE_HOME`" = "" ] &&  echo "la variable d'environnement $MY_HBASE_HOME doit être définie" && exit 1
            	classpath="$DIRECTORY_LIB/datagenerator.jar"
            	for file_jar in $(find $MY_HBASE_HOME/lib | grep jar)
            	do
            		classpath="${classpath}:$file_jar"
            	done
            	required_jar="$required_jar $DIRECTORY_LIB/datagenerator_hbaseext.jar?datagenerator.MainHbaseGenerator"
            	target_jar="$DIRECTORY_LIB/datagenerator_hbaseext.jar"
            fi
